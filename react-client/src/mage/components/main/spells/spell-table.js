...
class SpellTable extends React.Component {
    ...
    constructor(props) {
        ...
        this.arcanumUrl = this.baseUrl + "/mage/arcanum/all";
    ...
    async getData(query) {
        ...
        const getArcanum = data => this.setState({
            books: { arcanum: data },
        });

        var arcanum = null;
        
        await this.fetch(this.arcanumUrl + query, result => arcanum = result, this.retryFetch);
