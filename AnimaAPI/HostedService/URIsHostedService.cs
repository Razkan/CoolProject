...
namespace AnimaAPI.HostedService
{
    public class URIsHostedService : IHostedService
    {
        ...
        public async Task StartAsync(CancellationToken cancellationToken)
        {
            ...
            {
                var anima_host = Environment.GetEnvironmentVariable("ANIMA_HOST") ?? "localhost:9006";
                var tasks = new List<Task>
                {
                    rpc.Register<ICoreSpellBook>(new Uri($"http://{anima_host}/core/all")),
                    rpc.Register<IArcanaSpellBook>(new Uri($"http://{anima_host}/arcana/all")),
